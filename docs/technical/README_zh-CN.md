...
主题色性能数据放在技术说明里，而不是根 README：它对维护者判断热路径和回退策略有价值，但普通用户不需要先看到跑分。

当前可直接打开 [theme-benchmark.html](../../theme-benchmark.html) 复测。它会比较三条路径：`JS 32x32`、`JS 96x96` 和 `WASM 96x96`，并输出耗时表格和主题色结果。

以下数据来自本地壁纸 `安静氛围-森林.png`（`4096 x 3070`）、`200` 轮 benchmark、`20` 轮预热：

| 路径 | 采样 | 像素数 | 图像读取均值 | 分析均值 | 总耗时均值 | 总耗时中位数 | 总耗时范围 |
|------|------|--------|--------------|----------|------------|--------------|------------|
...
- `JS 32x32` 仍然很快，因为它只处理 1024 个像素，适合展示低采样成本。
- 同样 `96x96` 采样规模下，C++ / WASM 正式主题色引擎的平均总路径约为 JS fallback 的 `4.56x`。
- 单看分析部分，这轮 C++ / WASM engine 约为 JS fallback 的 `6.34x`。
- 这不是同一套算法的纯语言跑分。WASM 是正式主题色引擎，JS 是异常环境下的 fallback；两者采样规模相同，但权重、候选色评分和颜色桶处理不同，所以颜色输出可能不同。
- 两条路径都保留是有意设计：WASM 提供更完整、更快的主题色分析，JS fallback 保证网页模式或异常环境仍然可用。
