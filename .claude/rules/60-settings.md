...
- 数据导入控件应由统一状态驱动，不要在分散事件回调里分别修改按钮和输入框状态。
- 未选择文件时，导入按钮必须禁用。
- 选择 JSON 备份后，导入按钮可用，但导入口令输入和口令可见性按钮保持禁用。
- 只有选择 `.ptab` 备份后，才启用导入口令输入和口令显示/隐藏按钮。
- 加密导出口令和 `.ptab` 导入口令应使用同一套显示/隐藏逻辑；可见性按钮只显示眼睛/遮挡眼睛图标，复用现有口令无障碍标签和 `aria-pressed` 表达状态，不为显示/隐藏新增可见文案或 i18n key。
